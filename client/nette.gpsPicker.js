...
(function(window, undefined) {
	...
	var shapes = {
		point: (function() {
			var PointShape = function($el, $inputs, $search) {
				this._el = $el;
				this._latInput = $inputs.filter('[id$=lat]');
				this._lngInput = $inputs.filter('[id$=lng]');
				this._searchInput = $search;
			};
			PointShape.prototype.fill = function(lat, lng, address) {
				this._latInput.val(lat);
				this._lngInput.val(lng);

				if (this._searchInput !== null && address !== null) {
					this._searchInput.val(address);
				}
								...
								if (options.search) {
									...
									geocoder.geocode({'latLng': e.latLng}, function(results, status) {
										if (status === google.maps.GeocoderStatus.OK) {
											shape.fill(e.latLng.lat(), e.latLng.lng(), results[0].formatted_address);
										}
									});
								} else {
									shape.fill(e.latLng.lat(), e.latLng.lng());
	...
	var GpsPicker = function() {
		...
		this.initialize = function(el, options) {
			var $el = $(el), gpspicker;
			if (gpspicker = $el.data('gpspicker'))
				return gpspicker;
			...
			var shape = new shapes[options.shape]($el, $inputs, $search);
			shape.map = map;
